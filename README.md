...
- 进行中：M2.01 订阅 SKU + 5 态订单状态机（pending/paid/active/cancelled/expired，含回调幂等、退订/续费/到期）
- 待做：M2.02 微信支付接入（需商户号，当前用 StubPaymentProvider 兜底，接真支付时替换 DI）、飞书多维表格接入、小程序端
