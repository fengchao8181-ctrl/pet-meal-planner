(function () {
  ...
  function renderPlan(plan) {
    const el = $('#planResult');
    const parts = [];
    parts.push(`<div class="result-title">${esc(plan.pet.name)} 的${esc(plan.pet.modeLabel || plan.modeLabel)}餐单</div>`);
    parts.push(`<p class="result-sub">${esc(plan.pet.breed)} · ${Number(plan.pet.weightKg) || 0}kg · ${esc(plan.pet.bodyType)}体型 · ${esc(plan.pet.stage)} · 规则引擎 v${esc(plan.ruleVersion)}</p>`);
    parts.push(`<div class="kcal-row"><span>每日所需热量（DER）</span><b>${Number(plan.derKcal) || 0} kcal</b></div>`);
    ...
    if (plan.kibble) {
      ...
      if (plan.kibble.gramsPerDay) {
        parts.push(`<p class="note">按标签热量换算，主粮建议每日 <b>${Number(plan.kibble.gramsPerDay) || 0} g</b>（约占总热量 ${plan.mode === 'B' ? '90%' : '70%'}）</p>`);
      }
      parts.push('</div>');
    }
    ...
    if (plan.supplement && plan.supplement.length > 0) {
      ...
      plan.supplement.forEach((s) => {
        parts.push(`<tr><td>${esc(s.name)}</td><td><b>${Number(s.grams) || 0} g</b></td><td>${purposeLabel(s.purpose)}</td></tr>`);
      });
    ...
    if (plan.freshMeals) {
      ...
      parts.push(`<div class="block"><h3>${isPure ? '🍖 每日鲜食配方' : '🥘 鲜食顿次配方（每周 ' + esc(plan.freshMeals.length) + ' 种搭配）'}</h3>`);
      parts.push('<table class="plan-table"><thead><tr><th>食材</th><th>克数</th><th>热量</th></tr></thead><tbody>');
      plan.freshMeals.forEach((m) => {
        parts.push(`<tr><td>${esc(m.name)}</td><td><b>${Number(m.grams) || 0} g</b></td><td>${Number(m.kcal) || 0} kcal</td></tr>`);
      });
      parts.push('</tbody></table></div>');
    }
